...
const FooterMobileMenu = ({ selected }) => {
  ...
  useEffect(() => {
    if (cartItems?.cartmasters) {
      const length = cartItems.cartmasters.reduce(
        (acc, cart) => acc + (cart.foods?.length || 0),
        0,
      );
      setCartLength(length);
    }
  ...
  useEffect(() => {
    ...
    const mainFooters = document.querySelectorAll(
      "footer:not(.mobile-menu), .site-footer, .main-footer, .page-footer",
    );
        ...
        >
          ...
          >
            ...
            >
              ...
              />
              <span
                style={{ fontSize: "12px", fontWeight: "500", color: "lightgray" }}
              >
                Home
              </span>
            </Link>
          ...
          <li
            className={selected === "setting" ? `active` : ""}
            style={{ flex: 1 }}
          >
            <Link
              to={"/setting"}
            ...
            >
              <i
                className="fa fa-user mb-2 pt-1"
                style={{
                  fontSize: "20px",
                  marginBottom: "4px",
                  color: "lightgray"
