...
const Cart = () => {
  ...
  // optional: per-row loading to prevent double taps
  const [rowLoading, setRowLoading] = useState({}); // { [cartdetailid]: true/false }

  const handleDecrement = async (index) => {
    const food = CartFoods[index];
    if (!food) return;

    const current = parseInt(quantities[index], 10) || 0;
    const next = Math.max(0, current - 1);

    // prevent spamming
    if (rowLoading[food.cartdetailid]) return;
    setRowLoading((s) => ({ ...s, [food.cartdetailid]: true }));
    ...
    try {
      if (next === 0) {
        // tell backend this line is removed
        await handleProceed(food, 0);

        // Remove from both CartFoods AND quantities arrays
        setCartFoods((prev) => {
        ...
        // Also update quantities array to remove the corresponding index
        setQuantities((prev) => prev.filter((_, i) => i !== index));

      } else {
        // normal decrement
        await handleProceed(food, next);
      }
    } catch (e) {
      // rollback on failure
      setQuantities((prev) => prev.map((q, i) => (i === index ? current : q)));
      console.error("Failed to update quantity:", e);
      toast.error("Could not update cart. Please try again.");
    } finally {
      setRowLoading((s) => ({ ...s, [food.cartdetailid]: false }));
    }
  };
  ...
  //     if (next === 0) {
  //       // tell backend this line is removed
  //       await handleProceed(food, 0); // <-- your API must treat 0 as delete

  //       // if your backend requires a dedicated delete, use this instead:
  //       // await axios.post(`${API_URL}/deletecartdetail`, {
  //       //   cartdetailid: food.cartdetailid,
  //       //   cartid: cartId,
  //       // });
  ...
  const Addorder = async (paymentMethod = paymentMode) => {
    setIsLoading(true);
    const paymentDateTime = getCurrentDateTime();
  ...
  const handleCashOnDeliveryClick = async () => {
    setIsLoading(true);
    try {
      await Addorder("Cash On Delivery"); // Pass payment mode directly
    } catch (error) {
  ...
  //   try {
  //     await Addorder(); // Directly place order
  //     // No need to navigate here since Addorder handles navigation
  //   } catch (error) {
  //     console.error("Cash order failed:", error);
  //   } finally {
  //     setIsLoading(false);
  ...
  // const handleCashOnDeliveryClick = async () => {
  //   setIsLoading(true);
  //   setPaymentMode("Cash On Delivery"); // Set payment mode first

  //   try {
  //     await Addorder(); // Now it will use "Cash On Delivery"
  //   } catch (error) {
  //     console.error("Cash order failed:", error);
  //   } finally {
  //     setIsLoading(false);
  ...
  // const handlePayOnlineClick = async () => {
  //   setIsLoading(true);
  //   setPaymentMode("Stripe"); // Set payment mode first

  //   try {
  //     await Addorder(); // Now it will use "Stripe"
  //   } catch (error) {
  //     console.error("Online payment failed:", error);
  //   } finally {
  //     setIsLoading(false);
                          ...
                          <li key={index}>
                            <div className="horizontal-product-box">
                              <div className="product-content cart-top">
                                ...
                                {/* <h6 className="ingredients-text small">
                                  {food.menutype}
                                  <br />
                                  {food.category}
                                  {food.carttopupdetails && food.carttopupdetails.length > 0 && (
                                    <div className="customizations-section">
                                      <div className="row">
                                        {food.carttopupdetails.map((topup, topupIndex) => (
                                          <div key={topupIndex} className="col-12">
                                            {topup.cartextratopupdetails.map((extra, extraIndex) => (
                                              <div key={extraIndex} className="small" style={{ display: "flex", alignItems: "start", justifyContent: "start", flexDirection: "column" }}>
                                                {extra.name}
                                              </div>
                                            ))}
                                          </div>
                                ...
                                <h6 className="ingredients-text small">
                                  ...
                                  {food.carttopupdetails && food.carttopupdetails.length > 0 && (
                                    <span>
                                      {" • "}
                                      {food.carttopupdetails.flatMap((topup, topupIndex) =>
                                        topup.cartextratopupdetails.map((extra, extraIndex) => (
                                          <span key={`${topupIndex}-${extraIndex}`}>
                                            {extra.name}
                                            {topupIndex < food.carttopupdetails.length - 1 ||
                                              extraIndex < topup.cartextratopupdetails.length - 1 ? ", " : ""}
                                          </span>
                                        ))
                                      )}
                                    </span>
                                ...
                                <div className="d-flex align-items-center justify-content-between mt-md-2 mt-1 gap-1">
                                  ...
                                  >
                                    <i
                                      className={parseInt(quantities[index], 10) === 1 ? "bi bi-trash3" : "ri-subtract-line"}
                                      onClick={() => handleDecrement(index)}
                                      style={{ cursor: "pointer", color: rowLoading[CartFoods[index]?.cartdetailid] ? "#aaa" : "red" }}
                                    />
                                    <input
                                      type="number"
                              ...
                              <div className="product-content cart-top">
                                <div className="d-flex align-items-center justify-content-between mb-2">
                                  <div className="flex-grow-1">
                                    <h5 className="mb-1">{food.food}</h5>

                                    <div className="d-flex align-items-center flex-wrap gap-2 mb-2">
                                      {food.isveg === "1" && (
                                        <span className="badge bg-success" style={{ fontSize: '10px' }}>VEG</span>
                                      )}
                                      {food.isnonveg === "1" && (
                                        <span className="badge bg-danger" style={{ fontSize: '10px' }}>NON-VEG</span>
                                      )}
                                    ...
                                    {food.description && (
                                      <p className="text-muted small mb-2" style={{ fontSize: '12px', lineHeight: '1.3' }}>
                                        {food.description}
                                      </p>
                                    )}
                                  </div>
                                  ...
                                  <div className="text-end">
                                    <h6 className="product-price mb-2">£{totalPrice.toFixed(2)}</h6>
                                    <div className="plus-minus" style={{ fontSize: "medium" }}>
                                      <i
                                        className={parseInt(quantities[index], 10) === 1 ? "bi bi-trash3" : "ri-subtract-line"}
                                        onClick={() => handleDecrement(index)}
                                        style={{
                                          cursor: "pointer",
                                          color: rowLoading[CartFoods[index]?.cartdetailid] ? "#aaa" : "red",
                                          fontSize: "14px"
                                        }}
                                      />
                                      <input
                                ...
                                {food.carttopupdetails && food.carttopupdetails.length > 0 && (
                                  <div className="customizations-section border-top pt-2">
                                    <div className="small text-muted mb-2 fw-semibold">Customizations:</div>
                                        ...
                                        <div key={topupIndex} className="col-12">
                                          <div className="d-flex justify-content-between align-items-start">
                                            <div className="flex-grow-1">
                                              ...
                                              {topup.cartextratopupdetails.map((extra, extraIndex) => (
                                                <div key={extraIndex} className="d-flex justify-content-between small">
                                                  <span className="text-muted ms-2">
                                                    {extra.name}
                                                  </span>
                                                  {extra.extratopupcost && parseFloat(extra.extratopupcost) > 0 && (
                                                    <span className="text-success fw-medium">
                                                      +£{parseFloat(extra.extratopupcost).toFixed(2)}
                                                    </span>
                                                  )}
                                                </div>
                                              ))}
                                            ...
                                            {topup.isrequired === "1" && (
                                              <span className="badge bg-warning text-dark small ms-2">
                                                Required
                                              </span>
                                            )}
                                          </div>
                                ...
                                <div className="price-breakdown border-top pt-2 mt-2">
                                  <div className="row small text-muted">
                                    <div className="col-6">
                                      Base price ({food.menutype}):
                                    </div>
                                    ...
                                    <div className="col-12 border-top mt-1 pt-1">
                                      <div className="d-flex justify-content-between">
                                        <span className="fw-medium">Item Total:</span>
                    ...
                    <div className="container">
                      <Button
                        ...
                        disabled={
                          (activeTab === "Delivery" &&
                            cartItem?.isdeliveryavailable !== "1") ||
                          (activeTab === "Delivery" && isCheckoutDisabled)
                        }
                      >
                        CHECKOUT
                      </Button>
                          ...
                          <div className="d-flex justify-content-center w-100">
                            ...
                            {cartItems?.paymentmethod?.includes("Card") && (
                              <Button
                                ...
                                onClick={async () => {
                                  setIsLoading(true); 
                                  await handlePayOnlineClick();
                                  setIsLoading(false); 
                                }}
                                disabled={isLoading || buttonText === "CONFIRM ORDER"}
                              >
                        ...
                        <Modal.Body>
                          <div className="d-flex justify-content-center w-100 gap-2">
                            ...
                            {cartItems?.paymentmethod?.includes("Cash") && (
                              ...
                              >
                                {isLoading ? "Placing Order..." : "PAY BY CASH"}
                              </Button>
                            )}
