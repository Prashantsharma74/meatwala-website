...
const Myhistory = () => {
  ...
  const currentHistory = loyaltyHistory.slice(
    indexOfFirstItem,
    indexOfLastItem
  );

  const totalPages = Math.ceil(loyaltyHistory.length / itemsPerPage);
    ...
    const fetchLoyaltyHistory = async () => {
      try {
        const response = await axios.post(
          "https://partnermeatwala.com/api/customer/getcustloyaltyhistory",
          data
        );
        if (response.data.status === "1") {
          setLoyaltyHistory(response?.data?.details);
        } else {
                ...
                <div class="col-lg-12 d-flex justify-content-between mb-3">
                  <div className="title">
                    ...
                    <p style={{ fontSize: "12px" }}>
                      Select a store where you have points, shop and redeem them
                      at checkout.
                    </p>
                  </div>
                </div>
                <ul className="order-box-list">
                  ...
                  {currentHistory.length > 0 ? (
                    ...
                    <li className="order-box">
                      <p>No loyalty history available.</p>
                    </li>
                  )}
                </ul>
                {loyaltyHistory.length > itemsPerPage && (
