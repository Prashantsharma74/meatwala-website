...
export const addcustinfo = async (data) => {
  try {
    console.log("Add cust data:", data);
    const res = await axios.post(`${API_URL}/addcustinfo`, data, {
      headers: {
        "Content-Type": "application/json",
      },
    });
    console.log("Add cust response:", res.data);
    if (res.data.status === "1") {
      return res.data;
    }
  } catch (error) {
...
export const AddRestaurant = async (data) => {
  try {
    const response = await axios.post(
      `${API_URL1}/insertrestaurant`,
      data, // plain object
      { headers: { "Content-Type": "application/json" } }
    );

    if (response?.data?.success === "1") return response.data;
    throw new Error(response?.data?.returnmsg || "AddRestaurant failed");
  } catch (error) {
